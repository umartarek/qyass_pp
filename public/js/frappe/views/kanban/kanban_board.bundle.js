...
(function () {
	...
	frappe.views.KanbanBoardCard = function (card, wrapper) {
		...
		function init() {
			if (!card) return;
			make_dom();
            if (card.doctype !== "Organization Entry Records") {
                render_card_meta();
            }
		}
		...
		function make_dom() {
            ...
            const CARD_TEMPLATE = `<div class="kanban-card-wrapper {{ disable_click }}" data-name="{{encodeURIComponent(name)}}">
            ...
            if (card.doctype === "Organization Entry Records") {
                self.$card = $(frappe.render(CARD_TEMPLATE, opts)).appendTo(wrapper);
            }
            else {
		...
		function get_doc_content(card) {
            // for `Organization Entry Records` doctype
            if (card.doctype === "Organization Entry Records") {
                var fields = [];
                for (let field_name of cur_list.board.fields) {
                    if (field_name === "element") {continue;}
                    let field =
                    ...
                    if (field_name === "evaluation") {
                        const COLOR = {'التزام كلي': '#00E676', 'التزام جزئي': '#FFA500', 'عدم التزام': '#FF0000', 'لا ينطبق': '#808080'};
                        fields.push(`
                        <hr/>
                        <div class="text-muted" style="overflow-wrap: break-word;">
                            ${label}
                        <svg width="17" height="17">
                            <circle cx="8" cy="8" r="7" fill="${COLOR[value]}" />
                        </svg>
                            <span>${value}</span>
                        </div>
                    `);
                    }
                    else if (field_name === "recommendations" && value) {
                        fields.push(`
                        <hr/>
                        <div class="text-muted" style="overflow-wrap: break-word;">
                            ${label}
