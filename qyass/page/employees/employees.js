frappe.pages['employees'].on_page_load = function(wrapper) {
    console.log("--- Employee Directory: Initializing ---");

    // 1. Create the standard page structure
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'دليل الموظفين (Employee Directory)',
        single_column: true
    });
    ...
    new EmployeeDashboard(wrapper, page);
};
...
class EmployeeDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-employees { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
<!-- MISA Countdown Component End -->
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>لوحة الموظفين</h1>
                            <p>إدارة بيانات الموظفين، الإدارات، ومعلومات الاتصال.</p>
                            <!-- Changed to ID for event binding -->
                            <button class="hero-btn" id="btn-new-employee">
                                <i class="fa fa-plus-circle"></i> موظف جديد
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">إجمالي الموظفين</span>
                                </div>
                            </div>
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">موظف نشط</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="employee-list-wrapper" class="list-grid">
        ...
        this.body.find('#btn-new-employee').on('click', () => this.open_create_dialog());
    }
    ...
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                },
                {
                    ...
                    fieldname: 'type',
                    fieldtype: 'Select',
                    options: ['خارج الجهة', 'داخل الجهة','موظف تحول رقمي']
                },
                {
            ...
            primary_action_label: 'حفظ الموظف',
            primary_action: (values) => {
                this.create_employee(values, d);
            }
        });
        ...
        // Add Header Icon
        d.$wrapper.find('.modal-header').prepend('<div class="dialog-icon"><i class="fa fa-user-plus"></i></div>');

        d.show();
    }

    // --- NEW METHOD: Handle API Insert ---
    create_employee(values, dialog) {
        frappe.call({
            ...
            callback: (r) => {
                if (!r.exc) {
                    dialog.hide();
                    frappe.show_alert({
                        message: `<i class="fa fa-check-circle"></i> تم إضافة الموظف: ${r.message.employee_name}`,
                        indicator: 'green'
                    }, 5);
    ...
    fetch_data() {
        frappe.db.get_list('Employee', {
            ...
            console.error(err);
            this.body.find('#employee-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#employee-list-wrapper');
        ...
        data.forEach((item, index) => {
            ...
            if (status === 'Active') { statusColor = '#10b981'; statusBg = '#ecfdf5'; } 
            else if (status === 'Inactive' || status === 'Left') { statusColor = '#ef4444'; statusBg = '#fef2f2'; } 
            else if (status === 'Suspended') { statusColor = '#f59e0b'; statusBg = '#fffbeb'; } 

            let typeBadge = '';
            if(item.type) {
                typeBadge = `<span class="type-badge">${item.type}</span>`;
            }

            const targetUrl = `/app/employee/${item.name}`;
                ...
                <div class="modern-list-item glass-card" 
                    ...
                    <div class="item-left">
                        ...
                        <div class="item-info">
                            ...
                            <div class="item-meta">
                                <span class="meta-tag job-tag">
                                    <i class="fa fa-briefcase"></i> ${job}
                                </span>
                                <span class="meta-text">
                                    <i class="fa fa-building"></i> ${dept}
                                </span>
                            </div>
                        </div>
                    </div>
    ...
    setup_styles() {
        if ($('#qyass-employee-theme-css').length) return;
        ...
        const css = `
            ...
            .job-tag { color: #0369a1; background: #e0f2fe; padding: 3px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600; }
            ...
            .type-badge { font-size: 0.7rem; color: #64748b; background: #f1f5f9; padding: 4px 8px; border-radius: 4px; }

            .btn-arrow { color: #cbd5e1; transition: 0.2s; margin-right: 10px; }
        ...
        $('<style id="qyass-employee-theme-css">').text(css).appendTo('head');
    }
}