frappe.pages['ig'].on_page_load = function(wrapper) {
    console.log("--- Internal Generalizations Dashboard: Initializing ---");

    // 1. Create the standard page structure
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'التعاميم الداخلية (Internal Generalizations)',
        single_column: true
    });
    ...
    new InternalGenDashboard(wrapper, page);
};

class InternalGenDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-ig { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>لوحة التعاميم الداخلية</h1>
                            <p>توجيه التعاميم لمسؤولي التحول الرقمي ومتابعة المحتوى.</p>
                            <!-- Changed to ID for event binding -->
                            <button class="hero-btn" id="btn-new-internal">
                                <i class="fa fa-plus-circle"></i> تعميم داخلي جديد
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">إجمالي التعاميم</span>
                                </div>
                            </div>
                            <div class="stat-card glass-card">
                                <div class="stat-icon"><i class="fa fa-paperclip"></i></div>
                                <div class="stat-info">
                                    <span class="stat-num" id="stat-attach">...</span>
                                    <span class="stat-desc">تعاميم بمرفقات</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="internal-list-wrapper" class="list-grid">
        ...
        this.body.find('#btn-new-internal').on('click', () => this.open_create_dialog());
    }

    // --- NEW METHOD: Create Dialog ---
    open_create_dialog() {
        const child_doctype = 'Digital Transformation Officer';

        // Fetch Child Metadata first
        frappe.model.with_doctype(child_doctype, () => {
            ...
            let d = new frappe.ui.Dialog({
                title: 'إصدار تعميم داخلي جديد',
                size: 'large',
                fields: [
                    {
                        label: 'بيانات التعميم',
                        fieldtype: 'Section Break'
                    },
                    {
                        label: 'Generalization Date',
                        fieldname: 'generalization_date',
                        fieldtype: 'Date',
                        default: frappe.datetime.now_date()
                    },
                    {
                        label: 'Generalization Subject',
                        fieldname: 'generalization_subject',
                        fieldtype: 'Data',
                        reqd: 1,
                    ...
                    {
                        label: 'Generalization Recipient',
                        fieldname: 'generalization_recipient',
                        fieldtype: 'Table', // Rendered as Table for dialog stability
                    ...
                    {
                        label: 'Generalization Content',
                        fieldname: 'generalization_content',
                        fieldtype: 'Text', // Or 'Text Editor'
                        reqd: 1
                    },
                    {
                        label: 'Generalization Attachment',
                        fieldname: 'generalization_attachment',
                        fieldtype: 'Attach'
                    }
                ],
                primary_action_label: 'إرسال التعميم',
                primary_action: (values) => {
                    this.create_internal_gen(values, d);
                }
            });
            ...
            // Add Header Icon
            d.$wrapper.find('.modal-header').prepend('<div class="dialog-icon"><i class="fa fa-network-wired"></i></div>');

            d.show();
            
            // Style grid
            d.$wrapper.find('.form-grid').css({'min-height': '150px', 'background': '#fff'});
        });
    }

    // --- NEW METHOD: Handle API Insert ---
    create_internal_gen(values, dialog) {
        frappe.call({
            method: 'frappe.client.insert',
            args: {
                doc: {
                    doctype: 'Internal Generalizations',
                    ...values
                }
            },
            freeze: true,
            ...
            callback: (r) => {
                if (!r.exc) {
                    dialog.hide();
                    frappe.show_alert({
                        message: `<i class="fa fa-check-circle"></i> تم حفظ التعميم بنجاح`,
                        indicator: 'green'
                    }, 5);
    ...
    fetch_data() {
        // Fetching main fields
        frappe.db.get_list('Internal Generalizations', {
            fields: [
                'name', 
                'generalization_subject', 
                'generalization_date', 
                'generalization_attachment',
                'generalization_content'
            ],
            limit: 200,
            order_by: 'generalization_date desc'
        }).then(data => {
            this.render_list(data);
        }).catch(err => {
            console.error(err);
            this.body.find('#internal-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#internal-list-wrapper');
        const statCount = this.body.find('#stat-count');
        const statAttach = this.body.find('#stat-attach');
        ...
        const attachCount = data.filter(item => item.generalization_attachment).length;
        
        statCount.text(count);
        statAttach.text(attachCount);
        listBadge.text(count);

        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-inbox" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد تعاميم داخلية</h3>
                    <p>قم بإنشاء التعميم الأول</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            // Data Parsing
            const subject = item.generalization_subject || item.name;
            const date = frappe.datetime.str_to_user(item.generalization_date);
            const hasAttachment = item.generalization_attachment ? true : false;
            
            // Content Preview (Strip HTML and truncate)
            let contentPreview = item.generalization_content || '';
            let div = document.createElement("div");
            div.innerHTML = contentPreview;
            ...
            let attachBadge = '';
            if(hasAttachment) {
                attachBadge = `<span class="attach-badge"><i class="fa fa-paperclip"></i> مرفق</span>`;
            }

            // Construct URL
            const targetUrl = `/app/internal-generalizations/${item.name}`;

            // Render Card with onclick
            const card = $(`
                <div class="modern-list-item glass-card" 
                    ...
                    <div class="item-left">
                        <div class="item-icon-box">
                            <span style="font-size: 1.2rem;">${moment(item.generalization_date).format('DD')}</span>
                        </div>
                        <div class="item-info">
                            <h3 class="item-title">${subject}</h3>
                            <div class="item-meta">
                                <span class="meta-tag date-tag">
                                    <i class="fa fa-calendar-alt"></i> ${date}
                                </span>
                                <span class="meta-text" style="color:#94a3b8; font-size:0.8rem">
                                    ${textContent}
                                </span>
                            </div>
                        </div>
    ...
    setup_styles() {
        if ($('#qyass-internal-gen-theme-css').length) return;
        ...
        const css = `
            ...
            .date-tag { color: #0369a1; background: #e0f2fe; padding: 3px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600; }
            ...
            .qyass-theme-dialog .grid-heading-row {
                background: #f0f9ff; color: #0369a1; font-weight: 700; border-bottom: 1px solid #bae6fd;
            }
        ...
        $('<style id="qyass-internal-gen-theme-css">').text(css).appendTo('head');
    }
}