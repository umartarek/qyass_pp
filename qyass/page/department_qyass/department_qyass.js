frappe.pages['department-qyass'].on_page_load = function (wrapper) {
    ...
    async function fetchDataAndDrawCharts() {
        try {
            const response = await fetch('https://qyass.newerasofts.com/api/method/qyass.api.charts.department.get_completed_sum');
            ...
            if (data.message) {
                ...
                apiData.sort((a, b) => parseInt(a.department.split(':')[0]) - parseInt(b.department.split(':')[0]));
                ...
                apiData.forEach((item, index) => {
                    ...
                    section.innerHTML = `
                        ...
                        <div class="chart-container">
                            ...
                            <canvas id="donut_completed_${sanitizeName(department, index)}"></canvas>
                        </div>
                        <div class="chart-container">

                            <canvas id="donut_reviewed_${sanitizeName(department, index)}"></canvas>
                        </div>
                        <div class="stats">
                            <h3>المكتمل: ${completed}</h3>
                        </div>
                        <div class="stats">
                            <h3>المراجَع: ${reviewed}</h3>
                        </div>
                        <div class="stats">
                            <h3>الكل: ${total}</h3>
                        </div>
                    ...
                    // Draw donut charts
                    drawDonutChart(`donut_completed_${sanitizeName(department, index)}`, 'المكتمل %', completedPercentage, '#4caf50');
                    drawDonutChart(`donut_reviewed_${sanitizeName(department, index)}`, 'المراجَع %', reviewedPercentage, '#2196f3');
                });
            } else {
