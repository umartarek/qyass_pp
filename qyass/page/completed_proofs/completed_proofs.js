frappe.pages['completed-proofs'].on_page_load = function(wrapper) {
    console.log("--- Qyass Completed Proofs Report: Initializing ---");

    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'مستندات مكتملة لكل معيار',
        single_column: true
    });
    ...
    new CompletedProofsDashboard(wrapper, page);
};

class CompletedProofsDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-completed-proofs { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <!-- Report Summary Cards (Horizontal Scroll) -->
                    <div class="summary-cards-wrapper fade-in-up">
                        <div id="summary-cards-container" class="summary-cards-scroll">
                            <div class="text-center w-100 p-4 text-muted"><i class="fa fa-spinner fa-spin"></i> جاري تحميل الملخص...</div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card mb-4 chart-wrapper fade-in-up" style="padding: 25px; min-height: 420px; animation-delay: 0.1s;">
                        <div class="chart-header">
                            <h3 style="margin: 0; font-size: 1.2rem; font-weight: 800; color: #0c4a6e;">
                                <i class="fa fa-chart-bar text-primary mr-2"></i> الإحصائيات العامة للمعايير
                            </h3>
                        </div>
    ...
    fetch_data() {
        // Equivalent to your SQL: select element, sum(completed_count), sum(proof_count) group by element
        frappe.call({
    ...
    process_and_render_data(raw_data) {
        ...
        this.current_data = raw_data.map(item => {
            ...
            // Truncate(sum/sum, 2) * 100 logic
            let percent = total > 0 ? Math.floor((completed / total) * 100) : 0;

            return {
                element: item.element || 'بدون اسم',
                total: total,
                completed: completed,
                percent: percent
            };
        ...
        data.forEach((item, index) => {
            ...
            let card = $(`
                <div class="summary-card" style="animation: slideInRight 0.5s ease forwards; animation-delay: ${delay}ms; opacity: 0;">
                    <div class="summary-title" title="${item.element}">
                        <i class="fa fa-cube mr-1"></i> ${item.element.length > 20 ? item.element.substring(0,20)+'...' : item.element}
                    </div>
                    <div class="summary-stats">
                        ...
                        <div class="stat-box">
                            <span class="stat-label">مطلوب</span>
    ...
    render_dashboard(data) {
        ...
        data.forEach((item, index) => {
            // Chart Data Prep
            chart_labels.push(item.element.length > 25 ? item.element.substring(0, 25) + '...' : item.element);
            ds_completed.push(item.completed);
            ds_total.push(item.total);
            ds_percent.push(item.percent);

            // Table Row Prep
            // Using Blue monochromatic badges for percentage
            ...
            const row = $(`
                <tr style="animation: fadeInRow 0.3s ease forwards; animation-delay: ${index * 15}ms; opacity: 0;">
                    <td class="font-weight-bold" style="color: #0f172a; font-size: 1.05rem;">
                        <i class="fa fa-link mr-2 text-blue-muted"></i> ${item.element}
                    </td>
    ...
    render_chart(labels, completed, total, percent) {
        ...
        // Strict Blue Theme Colors: Dark Blue, Standard Blue, Light Sky Blue
        this.chart_instance = new frappe.Chart(container, {
            data: {
                labels: labels,
                datasets: [
                    { name: 'مستندات مكتملة', values: completed },
                    { name: 'اجمالي المستندات المطلوبه', values: total },
            ...
            type: 'bar',
            height: 420, 
            colors: ['#0284c7', '#0c4a6e', '#7dd3fc'], // Only shades of blue
            barOptions: {
                spaceRatio: 0.25, 
                radius: 4 
            },
    ...
    export_to_csv() {
        ...
        csvContent += "معيار,مستندات مكتملة,اجمالي المستندات المطلوبة,النسبه\n"; 
        ...
        link.setAttribute("download", `مستندات_مكتملة_${frappe.datetime.nowdate()}.csv`);
    ...
    setup_styles() {
        if ($('#qyass-proofs-css').length) return;

        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@300;400;500;700;800&display=swap');

            /* --- Base Monochromatic Blue Theme --- */
            ...
            .glass-header {
                ...
                border-bottom: 1px solid rgba(186, 230, 253, 0.5); /* Blue-ish border */
            ...
            .badge-blue-solid { background: #0284c7; color: white; box-shadow: 0 2px 8px rgba(2,132,199,0.3); } /* 100% Complete */
            .badge-blue-light { background: #e0f2fe; color: #0284c7; border: 1px solid #bae6fd; } /* Less than 100% */
            
            /* --- Chart Overrides (Blue Theme) --- */
        ...
        $('<style id="qyass-proofs-css">').text(css).appendTo('head');
    }
}