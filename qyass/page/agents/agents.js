frappe.pages['agents'].on_page_load = function(wrapper) {
    console.log("--- Qyass Agencies: Initializing Full Operations ---");

    // 1. إنشاء الهيكل الأساسي للصفحة
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'الوكالات (Agencies)',
        single_column: true
    });
    ...
    // 3. تشغيل واجهة الوكالات
    new QyassAgencyDashboard(wrapper, page);
};

class QyassAgencyDashboard {
    ...
    force_kiosk_mode() {
        const kioskStyle = `
			...
			#page-agents{padding-top: 0px !important;}
            .page-container {
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>دليل الوكالات</h1>
                            <p>إدارة واستعراض الوكالات المسجلة في النظام.</p>
                            <button class="hero-btn" id="btn-new-agency">
                                <i class="fa fa-plus-circle"></i> إضافة وكالة جديدة
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">وكالة مسجلة</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div class="search-box glass-card">
                            ...
                            <input type="text" id="agency-search" placeholder="بحث عن وكالة...">
                        </div>

                        <div id="agency-list-wrapper" class="list-grid">
        ...
        // Client-side Search
        layout.find('#agency-search').on('keyup', (e) => {
        ...
        // Open Create Dialog
        this.body.find('#btn-new-agency').on('click', () => this.open_create_dialog());
    }
    ...
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            ...
            fields: [
                { label: 'اسم الوكالة', fieldname: 'agency', fieldtype: 'Data', reqd: 1, placeholder: 'أدخل اسم الوكالة هنا' }
            ],
            primary_action_label: 'حفظ',
            primary_action: (values) => {
                frappe.call({
                    method: 'frappe.client.insert',
                    args: { doc: { doctype: 'Agency', agency: values.agency } },
                    freeze: true,
                    freeze_message: 'جاري الحفظ...',
                    callback: (r) => {
                        if (!r.exc) {
                            d.hide();
                            frappe.show_alert({ message: `تمت الإضافة: ${r.message.agency}`, indicator: 'green' }, 3);
                            this.fetch_data();
                        }
                    }
                });
    ...
    // 2. DATA EDIT (UPDATE)
    // ==========================================
    open_edit_dialog(old_name) {
        let d = new frappe.ui.Dialog({
            title: 'تعديل اسم الوكالة',
            fields: [
                { label: 'اسم الوكالة الحالي', fieldname: 'old_agency', fieldtype: 'Data', read_only: 1, default: old_name },
                { label: 'الاسم الجديد', fieldname: 'new_agency', fieldtype: 'Data', reqd: 1, default: old_name }
            ],
            primary_action_label: 'تحديث البيانات',
            primary_action: (values) => {
                if(values.old_agency === values.new_agency) {
                    d.hide(); return; // لا يوجد تغيير
                }
                frappe.call({
                    method: 'frappe.rename_doc',
                    args: { doctype: 'Agency', old: values.old_agency, new: values.new_agency, merge: 0 },
                    freeze: true,
                    freeze_message: 'جاري التحديث...',
                    callback: (r) => {
                        if (!r.exc) {
                            d.hide();
    ...
    // ==========================================
    open_preview_dialog(docname, date) {
        let d = new frappe.ui.Dialog({
            title: 'تفاصيل الوكالة',
            fields: [
                { label: 'اسم الوكالة', fieldname: 'agency', fieldtype: 'Data', read_only: 1, default: docname },
                { label: 'تاريخ آخر تحديث', fieldname: 'modified', fieldtype: 'Data', read_only: 1, default: date }
            ]
            // تمت إزالة primary_action لمنع فتح السجل الكامل
        });
        
        this.apply_dialog_theme(d, 'fa-eye');
        d.show();
        
        // إخفاء الفوتر السفلي (الذي يحتوي أزرار الإلغاء والحفظ) لجعل النافذة للاستعراض فقط
    ...
    delete_agency(agency_name) {
        frappe.confirm(`هل أنت متأكد من حذف الوكالة: <b>${agency_name}</b>؟`, () => {
            frappe.call({
                method: 'frappe.client.delete',
                args: { doctype: 'Agency', name: agency_name },
                freeze: true,
    ...
    // --- Fetch Data ---
    fetch_data() {
        frappe.db.get_list('Agency', {
            fields: ['name', 'agency', 'modified'],
            limit: 500,
            ...
            console.error(err);
            this.body.find('#agency-list-wrapper').html(`<div class="alert alert-danger">خطأ: ${err.message}</div>`);
        });
    }

    // --- Render List ---
    render_list(data) {
        const container = this.body.find('#agency-list-wrapper');
        ...
        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    ...
                    <p>النظام خالي من الوكالات حالياً</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            const title = item.agency || item.name;
            ...
            const card = $(`
                <div class="modern-list-item glass-card" style="animation-delay: ${delay}ms">
                    <div class="item-left clickable-area" data-name="${item.name}" data-date="${date}">
                    ...
                    <div class="item-actions">
                        <button class="action-icon-btn btn-view" data-name="${item.name}" data-date="${date}" title="استعراض سريع">
                            <i class="fa fa-eye"></i>
                        </button>
                        <button class="action-icon-btn btn-edit" data-name="${item.name}" title="تعديل">
                            <i class="fa fa-pen"></i>
                        </button>
                        <button class="action-icon-btn btn-delete" data-name="${item.name}" title="حذف">
            ...
            card.find('.item-left, .btn-view').on('click', (e) => {
                e.stopPropagation();
                this.open_preview_dialog($(e.currentTarget).attr('data-name'), $(e.currentTarget).attr('data-date'));
            });

            card.find('.btn-edit').on('click', (e) => {
                e.stopPropagation();
                this.open_edit_dialog($(e.currentTarget).attr('data-name'));
            });

            card.find('.btn-delete').on('click', (e) => {
                e.stopPropagation(); 
                this.delete_agency($(e.currentTarget).attr('data-name'));
            });
    ...
    setup_styles() {
        if ($('#qyass-agency-css').length) return;
        ...
        `;
        $('<style id="qyass-agency-css">').text(css).appendTo('head');
    }
}