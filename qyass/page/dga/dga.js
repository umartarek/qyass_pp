frappe.pages['dga'].on_page_load = function(wrapper) {
    console.log("--- DGA Generalization Dashboard: Initializing ---");

    // 1. Create the standard page structure
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'التعاميم (Generalizations)',
        single_column: true
    });
    ...
    new GeneralizationDashboard(wrapper, page);
};

class GeneralizationDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-dga { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>لوحة التعاميم الإدارية</h1>
                            <p>استعراض التعاميم الصادرة والمرفقات الرسمية.</p>
                            <!-- Changed to ID for event binding -->
                            <button class="hero-btn" id="btn-new-generalization">
                                <i class="fa fa-plus-circle"></i> تعميم جديد
                            </button>
                            <button class="hero-btn" style="background:#0ea5e9;margin-right:10px;" onclick="location.href='https://dga.gov.sa/ar/regulatory_framework/guidelines-and-methodologies-overview'">
                                <i class="fa fa-circle-exclamation"></i> الادلة الارشادية و المنهجيات
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">عدد التعاميم</span>
                                </div>
                            </div>
                            <div class="stat-card glass-card">
                                <div class="stat-icon"><i class="fa fa-paperclip"></i></div>
                                <div class="stat-info">
                                    <span class="stat-num" id="stat-attachments">...</span>
                                    <span class="stat-desc">مرفقات متاحة</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="generalization-list-wrapper" class="list-grid">
        ...
        this.body.find('#btn-new-generalization').on('click', () => this.open_create_dialog());
    }

    // --- NEW METHOD: Create Dialog ---
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            title: 'إصدار تعميم جديد',
            fields: [
                {
                    label: 'معلومات التعميم',
            ...
            primary_action_label: 'حفظ التعميم',
            primary_action: (values) => {
                this.create_generalization(values, d);
            }
        });
        ...
        // Add Header Icon
        d.$wrapper.find('.modal-header').prepend('<div class="dialog-icon"><i class="fa fa-bullhorn"></i></div>');

        d.show();
    }

    // --- NEW METHOD: Handle API Insert ---
    create_generalization(values, dialog) {
        frappe.call({
            method: 'frappe.client.insert',
            args: {
                doc: {
                    doctype: 'Dga Generalization',
                    ...values
                }
            },
            freeze: true,
    ...
    fetch_data() {
        frappe.db.get_list('Dga Generalization', {
            fields: [
                'name', 
            ...
            console.error(err);
            this.body.find('#generalization-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#generalization-list-wrapper');
        const statCount = this.body.find('#stat-count');
        const statAttachments = this.body.find('#stat-attachments');
        const listBadge = this.body.find('#list-badge');
        
        container.empty();

        const count = data.length;
        const attachmentCount = data.filter(item => item.generalization_attachment).length;
        
        statCount.text(count);
        statAttachments.text(attachmentCount);
        listBadge.text(count);

        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-folder-open" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد تعاميم</h3>
                    <p>ابدأ بإضافة التعميم الأول</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            if(hasAttachment) {
                ...
                attachBadge = `<span class="no-attach-badge">بدون مرفق</span>`;
            }

            const targetUrl = `/app/dga-generalization/${item.name}`;
    ...
    setup_styles() {
        if ($('#qyass-generalization-theme-css').length) return;
        ...
        const css = `
            ...
            .no-attach-badge {
                padding: 4px 12px; background: #f1f5f9; color: #94a3b8;
        ...
        $('<style id="qyass-generalization-theme-css">').text(css).appendTo('head');
    }
}