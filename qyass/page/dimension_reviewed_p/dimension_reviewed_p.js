frappe.pages['dimension-reviewed-p'].on_page_load = function(wrapper) {
    console.log("--- Qyass Dimension Reviewed Report: Initializing ---");

    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'تقرير مستندات تم مراجعتها لكل محور',
        single_column: true
    });
    ...
    new DimensionReviewedDashboard(wrapper, page);
};

class DimensionReviewedDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-dimension-reviewed-p { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="summary-cards-wrapper fade-in-up">
                        <div id="summary-cards-container" class="summary-cards-scroll">
                            <div class="text-center w-100 p-4 text-muted"><i class="fa fa-spinner fa-spin"></i> جاري تحميل ملخص المراجعات...</div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card mb-4 chart-wrapper fade-in-up" style="padding: 25px; min-height: 420px; animation-delay: 0.1s;">
                        <div class="chart-header">
                            <h3 style="margin: 0; font-size: 1.2rem; font-weight: 800; color: #0c4a6e;">
                                <i class="fa fa-chart-bar text-primary mr-2"></i> إحصائيات المراجعة للمحاور
                            </h3>
                        </div>
                    ...
                    <div class="glass-table-wrapper fade-in-up" style="animation-delay: 0.2s;">
                        <table class="qyass-table">
                            <thead>
                                <tr>
                                    <th width="40%">المحور</th>
                                    <th width="20%" class="text-center">تمت مراجعتها</th>
                                    <th width="20%" class="text-center">اجمالي المستندات</th>
                                    <th width="20%" class="text-center">نسبة المراجعة</th>
                                </tr>
                            </thead>
        ...
        frappe.call({
            ...
            args: {
                ...
                fields: [
                    ...
                    'sum(reviewed_count) as reviewed_sum' // Changed from proof_completed_sum
                ],
                group_by: 'dimension',
    ...
    process_and_render_data(raw_data) {
        ...
        this.current_data = raw_data.map(item => {
            ...
            let reviewed = parseFloat(item.reviewed_sum) || 0;
            // truncate(sum(reviewed_count)/sum(proof_count),2)*100
            let percent = total > 0 ? Math.floor((reviewed / total) * 100) : 0;

            return {
                dimension: item.dimension || 'بدون محور',
                total: total,
                reviewed: reviewed,
                percent: percent
            };
        ...
        data.forEach((item, index) => {
            ...
            let card = $(`
                <div class="summary-card" style="animation: slideInRight 0.5s ease forwards; animation-delay: ${delay}ms; opacity: 0;">
                    <div class="summary-title" title="${item.dimension}">
                        <i class="fa fa-layer-group mr-1"></i> ${item.dimension.length > 20 ? item.dimension.substring(0,20)+'...' : item.dimension}
                    </div>
    ...
    render_dashboard(data) {
        ...
        data.forEach((item, index) => {
            chart_labels.push(item.dimension.length > 25 ? item.dimension.substring(0, 25) + '...' : item.dimension);
            ...
            const row = $(`
                <tr style="animation: fadeInRow 0.3s ease forwards; animation-delay: ${index * 15}ms; opacity: 0;">
                    <td class="font-weight-bold" style="color: #0f172a; font-size: 1.05rem;">
                        <i class="fa fa-link mr-2 text-blue-muted"></i> ${item.dimension}
                    </td>
    ...
    render_chart(labels, reviewed, total, percent) {
        ...
        this.chart_instance = new frappe.Chart(container, {
            data: {
                labels: labels,
                datasets: [
                    { name: 'تمت مراجعتها', values: reviewed },
                    { name: 'اجمالي المستندات', values: total },
                    { name: 'النسبه', values: percent }
                ]
            },
            title: '',
            type: 'bar',
    ...
    export_to_csv() {
        ...
        csvContent += "المحور,تمت مراجعتها,اجمالي المستندات,نسبة المراجعة\n"; 
        ...
        link.setAttribute("download", `تقرير_المستندات_المراجعة_${frappe.datetime.nowdate()}.csv`);
    ...
    setup_styles() {
        if ($('#qyass-reviewed-css').length) return;
        ...
        $('<style id="qyass-reviewed-css">').text(css).appendTo('head');
    }
}