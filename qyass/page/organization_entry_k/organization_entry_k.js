frappe.pages['organization-entry-k'].on_page_load = function(wrapper) {
	...
	$("<div class='row organization-entry-k'></div>").appendTo(
		page.main
	);

	wrapper.elements_page_filter = new OrganizationEntryKanban(wrapper);

	page.body.addClass("no-border")
}


class OrganizationEntryKanban {
	constructor(wrapper) {
		this.wrapper = wrapper;
		this.page = wrapper.page;
		this.body = $(this.wrapper).find(".organization-entry-k");
		this.make();
	}
	...
	setup_page() {
		this.evaluation_select = this.wrapper.page.add_field({
			...
			options: '\nالتزام كلي\nالتزام جزئي\nعدم التزام\nلا ينطبق',
			change: () => {
				frappe.set_route("organization-entry-k", this.get_evaluation());
				this.set_form_route();
			},
		});
