frappe.pages['planning'].on_page_load = function(wrapper) {
    console.log("--- Qyass Plans Dashboard: Initializing ---");
    ...
    new QyassPlanDashboard(wrapper, page);
};

class QyassPlanDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-planning { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>لوحة التخطيط الاستراتيجي</h1>
                            <p>جدولة ومتابعة تنفيذ معايير القياس والفترات الزمنية.</p>
                            <!-- Button ID for binding -->
                            <button class="hero-btn" id="btn-new-plan">
                                <i class="fa fa-plus-circle"></i> خطة جديدة
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">إجمالي الخطط</span>
                                </div>
                            </div>
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">خطط سارية</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="plan-list-wrapper" class="list-grid">
        ...
        this.body.find('#btn-new-plan').on('click', () => this.open_create_dialog());
    }

    // --- CLONED METHOD: Create Dialog with Child Table Fix ---
    open_create_dialog() {
        const child_doctype = 'Element2024 child';

        // 1. Fetch Child DocType Meta first (Same strategy as Governance code)
        frappe.model.with_doctype(child_doctype, () => {
            ...
            let d = new frappe.ui.Dialog({
                ...
                size: 'large', // Wider dialog for the table
                fields: [
                    {
                        label: 'معلومات الخطة',
                    ...
                    },
                    {
                        ...
                        // --- CRITICAL FIX: Explicitly pass child fields ---
                        fields: grid_fields,
                        cannot_add_rows: false,
                        in_place_edit: true
                    }
                ],
                primary_action_label: 'حفظ الخطة',
                primary_action: (values) => {
                    this.create_plan(values, d);
                }
            });
    ...
    // --- CLONED METHOD: Handle API Insert ---
    create_plan(values, dialog) {
        frappe.call({
            ...
            args: {
                doc: {
                    ...
                    element: values.element || [] // Pass the child table array
                }
            },
            freeze: true,
            ...
            callback: (r) => {
                if (!r.exc) {
                    dialog.hide();
                    frappe.show_alert({
                        message: `<i class="fa fa-check-circle"></i> تم حفظ الخطة: ${r.message.p_name}`,
                        indicator: 'green'
                    }, 5);
    ...
    fetch_data() {
        frappe.db.get_list('qyass plan', {
            fields: ['name', 'p_name', 'employee', 'start_date', 'end_date', 'actual_date'],
            limit: 200,
            ...
            console.error(err);
            this.body.find('#plan-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#plan-list-wrapper');
        const statCount = this.body.find('#stat-count');
        const statActive = this.body.find('#stat-active');
        ...
        const activeCount = data.filter(item => item.end_date && new Date(item.end_date) >= today).length;
        ...
        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-clipboard" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد خطط</h3>
                    <p>ابدأ بإنشاء خطة العمل الأولى</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            const title = item.p_name || item.name;
            ...
            let statusClass = 'status-default'; // Grey
            ...
            const targetUrl = `/app/qyass-plan/${item.name}`;
    ...
    setup_styles() {
        if ($('#qyass-plan-theme-css').length) return;
        ...
        const css = `
            ...
            .status-box { padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; }
        ...
        $('<style id="qyass-plan-theme-css">').text(css).appendTo('head');
    }
}