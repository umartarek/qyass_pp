frappe.pages['employee-completed-p'].on_page_load = function(wrapper) {
    console.log("--- Qyass Employee Completed Report: Initializing ---");

    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'توزيع المستندات المكتمله للموظفين',
        single_column: true
    });
    ...
    new EmployeeCompletedDashboard(wrapper, page);
};

class EmployeeCompletedDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-employee-completed-p { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="summary-cards-wrapper fade-in-up">
                        <div id="summary-cards-container" class="summary-cards-scroll">
                            <div class="text-center w-100 p-4 text-muted"><i class="fa fa-spinner fa-spin"></i> جاري تحميل ملخص إنجاز الموظفين...</div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card mb-4 chart-wrapper fade-in-up" style="padding: 25px; min-height: 420px; animation-delay: 0.1s;">
                        <div class="chart-header">
                            <h3 style="margin: 0; font-size: 1.2rem; font-weight: 800; color: #0c4a6e;">
                                <i class="fa fa-chart-bar text-primary mr-2"></i> إحصائيات الإنجاز للموظفين
                            </h3>
                        </div>
        ...
        frappe.call({
            ...
            args: {
                ...
                fields: [
                    ...
                    'sum(completed_count) as proof_reviewed_sum'
                ],
                group_by: 'digital_transformation_officer',
                filters: [['digital_transformation_officer','!=', ''],
                    ['digital_transformation_officer', '!=', 'umartarek@gmail.com']
                    ], // لجلب الموظفين فقط وتجاهل الحقول الفارغة
    ...
    process_and_render_data(raw_data) {
        if (!raw_data || raw_data.length === 0) {
            this.body.find('#summary-cards-container').html('<div class="p-3 text-muted">لا توجد بيانات للعرض</div>');
            this.body.find('#analytics-chart').empty();
            this.body.find('#report-table-body').html('<tr><td colspan="4" class="text-center p-5 text-muted">لا توجد بيانات مرتبطة بالموظفين</td></tr>');
            return;
        }

        this.current_data = raw_data.map(item => {
            let total = parseFloat(item.proof_count_sum) || 0;
            let reviewed = parseFloat(item.proof_reviewed_sum) || 0;
            let percent = total > 0 ? Math.floor((reviewed / total) * 100) : 0;

            return {
                employee: item.digital_transformation_officer || 'غير محدد',
                total: total,
                reviewed: reviewed,
                percent: percent
            };
        ...
        data.forEach((item, index) => {
            ...
            let card = $(`
                <div class="summary-card" style="animation: slideInRight 0.5s ease forwards; animation-delay: ${delay}ms; opacity: 0;">
                    <div class="summary-title" title="${item.employee}">
                        <i class="fa fa-user-tie mr-1"></i> ${item.employee.length > 20 ? item.employee.substring(0,20)+'...' : item.employee}
                    </div>
                    <div class="summary-stats">
                        <div class="stat-box">
                            <span class="stat-label">مكتملة</span>
                            <span class="stat-value text-blue-bright">${item.reviewed}</span>
                        </div>
                        <div class="stat-box">
    ...
    render_dashboard(data) {
        ...
        data.forEach((item, index) => {
            chart_labels.push(item.employee.length > 25 ? item.employee.substring(0, 25) + '...' : item.employee);
            ds_reviewed.push(item.reviewed);
            ...
            const row = $(`
                <tr style="animation: fadeInRow 0.3s ease forwards; animation-delay: ${index * 15}ms; opacity: 0;">
                    <td class="font-weight-bold" style="color: #0f172a; font-size: 1.05rem;">
                        <i class="fa fa-user mr-2 text-blue-muted"></i> ${item.employee}
                    </td>
                    <td class="text-center font-weight-bold" style="color: #0ea5e9; font-size: 1.1rem; background: rgba(14, 165, 233, 0.03);">${item.reviewed}</td>
        ...
        this.render_chart(chart_labels, ds_reviewed, ds_total, ds_percent);
    }

    render_chart(labels, reviewed, total, percent) {
        ...
        this.chart_instance = new frappe.Chart(container, {
            data: {
                labels: labels,
                datasets: [
                    { name: 'مستندات مكتملة', values: reviewed },
    ...
    export_to_csv() {
        ...
        csvContent += "الموظف,مستندات مكتملة,اجمالي المستندات,النسبة\n"; 
        
        this.current_data.forEach(item => {
            let row = [
                `"${item.employee || ''}"`,
                item.reviewed, item.total, `${item.percent}%`
            ].join(",");
            csvContent += row + "\n";
        });
        ...
        link.setAttribute("download", `تقرير_إنجاز_الموظفين_${frappe.datetime.nowdate()}.csv`);
    ...
    setup_styles() {
        if ($('#qyass-employee-css').length) return;
        ...
        $('<style id="qyass-employee-css">').text(css).appendTo('head');
    }
}