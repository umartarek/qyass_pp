frappe.pages['digital-officer-per'].on_page_load = function(wrapper) {
    console.log("--- Qyass Digital Officer Cards: Initializing ---");

    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'بطاقات أداء مسؤولي التحول الرقمي',
        single_column: true
    });
    ...
    new DigitalOfficerCards(wrapper, page);
};

class DigitalOfficerCards {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-digital-officer-per { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
<!-- MISA Countdown Component End -->
                ...
                <div class="dashboard-container">
                    ...
                    <div id="officers-cards-grid" class="cards-grid">
                        <div class="text-center w-100 p-5 text-muted loading-state" style="grid-column: 1 / -1;">
                            ...
                            <h4 class="mt-3 font-weight-bold" style="color: #0c4a6e;">جاري جلب وبناء بطاقات المسؤولين...</h4>
                        </div>
                    </div>
                    ...
                    <div id="drilldown-view" style="display: none;">
                        <div class="drilldown-header glass-card mb-4 p-4 d-flex justify-content-between align-items-center">
                            <div>
                                <h3 id="drilldown-title" class="m-0 text-blue-dark font-weight-bold">معايير المسؤول</h3>
                                <p class="m-0 mt-2 text-blue-muted">تفاصيل المعايير الخاصة بهذا المسؤول</p>
                            </div>
    ...
    fetch_data() {
        // Level 1: ملخص أولي لبناء البطاقات الخارجية
        frappe.call({
            ...
            args: {
                ...
                group_by: 'digital_transformation_officer',
                filters: [
                    ['Elements-2024', 'digital_transformation_officer', '!=', 'umartarek@gmail.com'],
                    ['Elements-2024', 'digital_transformation_officer', '!=', '']
                ], 
    ...
    process_and_render_cards(raw_data) {
        const grid = this.body.find('#officers-cards-grid');
        grid.empty();
        ...
        if (!raw_data || raw_data.length === 0) {
            grid.html(`
                <div class="text-center w-100 p-5 glass-card" style="grid-column: 1 / -1; color: #64748b;">
                    ...
                    <h4>لا توجد بيانات مسجلة لمسؤولي التحول الرقمي</h4>
                </div>
            `);
            return;
        }
        ...
        this.current_data = raw_data.map(item => {
            ...
            return {
                officer: item.digital_transformation_officer,
                elements: elements,
                total: total,
                completed: completed,
                reviewed: reviewed,
        ...
        this.current_data.forEach((item, index) => {
            ...
            let card = $(`
                <div class="officer-card glass-card fade-in-up clickable-card" style="animation-delay: ${delay}ms;">
                    <div class="card-header-area">
                        <div class="avatar"><i class="fa fa-user-tie"></i></div>
                        <div class="title-area">
                            <h3 class="officer-name" title="${item.officer}">${item.officer}</h3>
            ...
            card.on('click', () => this.show_drilldown(item.officer));
            grid.append(card);
        });
    }
    ...
    async show_drilldown(officer) {
        this.body.find('#officers-cards-grid').hide();
        ...
        this.body.find('#drilldown-title').text(`معايير المسؤول: ${officer}`);
        ...
        try {
            // 1. جلب أسماء المعايير المرتبطة بالمسؤول
            let list_res = await frappe.call({
                method: 'frappe.client.get_list',
                args: {
                    doctype: 'Elements-2024',
                    filters: { 'digital_transformation_officer': officer },
                    fields: ['name'],
            ...
            // 2. جلب التفاصيل الكاملة باستخدام Promise.all لتسريع العملية
    ...
    render_drilldown_elements(docs) {
        ...
        if (docs.length === 0) {
            content.html(`
                <div class="text-center w-100 p-5 glass-card" style="grid-column: 1 / -1; color: #64748b;">
                    ...
                    <h5>لا توجد تفاصيل معايير متاحة لهذا المسؤول.</h5>
                </div>
            `);
            return;
        }
        ...
        docs.forEach((doc, index) => {
            // حفظ في الذاكرة لـ Level 3
            this.current_elements_docs[doc.name] = doc;

            // --- العد الحقيقي الدقيق (Real Accurate Numbers) ---
            // 1. المتطلبات
    ...
    hide_drilldown() {
        ...
        this.body.find('#element-details-view').hide();
        this.body.find('#officers-cards-grid').fadeIn(300);
    ...
    export_to_csv() {
        ...
        csvContent += "المسؤول,عدد المعايير,إجمالي المستندات المطلوبة,المستندات المكتملة,المستندات المراجعة,النسبة\n"; 
        ...
        link.setAttribute("download", `بطاقات_المسؤولين_${frappe.datetime.nowdate()}.csv`);
    ...
    setup_styles() {
        if ($('#qyass-officer-cards-css').length) return;
        ...
        const css = `
            ...
            .avatar {
                ...
                font-size: 1.6rem; color: #0284c7; box-shadow: 0 4px 10px rgba(2,132,199,0.1);
            }
            .title-area { display: flex; flex-direction: column; gap: 6px; }
            .officer-name { margin: 0; font-size: 1.2rem; font-weight: 800; color: #082f49; line-height: 1.2;}
            ...
            .officer-card:hover .stat-box { background: white; border-color: #e0f2fe; }
            ...
            .drilldown-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; }
            .element-card { padding: 20px; display: flex; flex-direction: column; justify-content: space-between; border-left: 4px solid #0284c7; }
            ...
            .section-title { color: #0369a1; font-weight: 700; margin: 0 0 15px 0; font-size: 1.1rem; border-bottom: 1px solid #bae6fd; padding-bottom: 8px; display: flex; align-items: center; gap: 8px; }
            .items-list { display: flex; flex-direction: column; gap: 10px; }
            .list-item {height:auto; background: rgba(255, 255, 255, 0.8); padding: 12px; border-radius: 8px; border: 1px solid #e0f2fe; display: flex; align-items: start; gap: 10px; box-shadow: 0 2px 5px rgba(2, 132, 199, 0.05); transition: 0.2s; }
        ...
        $('<style id="qyass-officer-cards-css">').text(css).appendTo('head');
    }
}