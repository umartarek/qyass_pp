frappe.pages['employees-per'].on_page_load = function(wrapper) {
    console.log("--- Qyass Employee Cards: Initializing ---");

    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'بطاقات أداء الموظفين المسؤولين',
        single_column: true
    });
    ...
    new EmployeeCards(wrapper, page);
};
...
class EmployeeCards {
    constructor(wrapper, page) {
        ...
        this.current_data = [];
        this.current_elements_docs = {}; // الذاكرة المؤقتة لتسريع فتح المستوى الثالث
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-employees-per { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
...
<!-- MISA Countdown Component End -->
                ...
                <div class="dashboard-container">
                    ...
                    <div id="employees-cards-grid" class="cards-grid">
                        <div class="text-center w-100 p-5 text-muted loading-state" style="grid-column: 1 / -1;">
                            ...
                            <h4 class="mt-3 font-weight-bold" style="color: #0c4a6e;">جاري جلب وبناء بطاقات الموظفين...</h4>
                        </div>
                    </div>
                    ...
                    <div id="drilldown-view" style="display: none;">
                        <div class="drilldown-header glass-card mb-4 p-4 d-flex justify-content-between align-items-center">
                            <div>
                                <h3 id="drilldown-title" class="m-0 text-blue-dark font-weight-bold">معايير الموظف</h3>
                                <p class="m-0 mt-2 text-blue-muted">تفاصيل المعايير والمستندات المسندة لهذا الموظف</p>
                            </div>
    ...
    fetch_data() {
        ...
        frappe.call({
            ...
            args: {
                ...
                group_by: 'the_employee_in_charge',
                filters: [
                    ['Elements-2024', 'the_employee_in_charge', '!=', '']
                ], 
    ...
    process_and_render_cards(raw_data) {
        const grid = this.body.find('#employees-cards-grid');
        grid.empty();
        
        if (!raw_data || raw_data.length === 0) {
            grid.html(`
                <div class="text-center w-100 p-5 glass-card" style="grid-column: 1 / -1; color: #64748b;">
                    <i class="fa fa-users fa-3x mb-3" style="opacity:0.3; color: #0284c7;"></i>
                    <h4>لا توجد بيانات مسجلة للموظفين</h4>
                </div>
            `);
            return;
        }
        ...
        this.current_data = raw_data.map(item => {
            ...
            return {
                employee: item.the_employee_in_charge,
                elements: elements,
                total: total,
                completed: completed,
                reviewed: reviewed,
        ...
        this.current_data.forEach((item, index) => {
            ...
            let card = $(`
                <div class="employee-card glass-card fade-in-up clickable-card" style="animation-delay: ${delay}ms;">
                    <div class="card-header-area">
                        <div class="avatar"><i class="fa fa-user-circle"></i></div>
                        <div class="title-area">
                            <h3 class="employee-name" title="${item.employee}">${item.employee}</h3>
            ...
            card.on('click', () => this.show_drilldown(item.employee));
            grid.append(card);
        });
    }
    ...
    async show_drilldown(employee) {
        this.body.find('#employees-cards-grid').hide();
        ...
        this.body.find('#drilldown-title').text(`معايير الموظف: ${employee}`);
        ...
        try {
            // 1. جلب أسماء المعايير المسندة للموظف
            let list_res = await frappe.call({
                method: 'frappe.client.get_list',
                args: {
                    doctype: 'Elements-2024',
                    filters: { 'the_employee_in_charge': employee },
                    fields: ['name'],
        ...
        if (docs.length === 0) {
            content.html(`
                <div class="text-center w-100 p-5 glass-card" style="grid-column: 1 / -1; color: #64748b;">
                    ...
                    <h5>لا توجد تفاصيل معايير متاحة لهذا الموظف.</h5>
                </div>
            `);
            return;
        }
    ...
    hide_drilldown() {
        ...
        this.body.find('#element-details-view').hide();
        this.body.find('#employees-cards-grid').fadeIn(300);
    ...
    export_to_csv() {
        ...
        csvContent += "الموظف,عدد المعايير,إجمالي المستندات المطلوبة,المستندات المكتملة,المستندات المراجعة,النسبة\n"; 
        ...
        link.setAttribute("download", `بطاقات_الموظفين_${frappe.datetime.nowdate()}.csv`);
    ...
    setup_styles() {
        if ($('#qyass-employee-cards-css').length) return;
        ...
        const css = `
            ...
            .employee-name { margin: 0; font-size: 1.2rem; font-weight: 800; color: #082f49; line-height: 1.2;}
            ...
            .employee-card:hover .stat-box { background: white; border-color: #e0f2fe; }
        ...
        $('<style id="qyass-employee-cards-css">').text(css).appendTo('head');
    }
}