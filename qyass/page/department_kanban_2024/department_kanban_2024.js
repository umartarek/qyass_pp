frappe.pages['department-kanban-2024'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		...
		$("<div class='row parent-section'><div class='col department-section' style='overflow-y: auto;height: 750px;'></div><div class='col element-section' style='overflow-y: auto;height: 750px;'></div><div class='col  dashboard-section' style='overflow-y: auto;height: 750px;'></div></div>").appendTo(
			page.main
		);
	
		// create an object from kanban class // no idea whtat does elements_page_filter mean
		wrapper.elements_page_filter = new DepartmentKanban2024(wrapper);
...
class DepartmentKanban2024{
	...
	async render_departments() {
		const response = await frappe.call({
		  module: "qyass.qyass",
		  page: "department_kanban_2024",
		  method: "get_departments",
		});
	  
		// get departments
		this.departments = response.message.departments;
		if (this.departments.length > 0) {
		  // empty departments cards // no known effect
		  this.cards = [];
	  
		  // add new departments cards
		  for (let i = 0; i < this.departments.length; i++) {
			await new Promise((resolve) => {
			  $(`
				<div target="_blank" rel="noopener noreferrer" class="frappe-card department-card">
				  <span>${this.departments[i].name}</span>
				</div>
			  `).appendTo(this.body.find(".department-section"));
	...
	add_style() {
		this.style = `
			...
			.department-section,.element-section,.dashboard-section {
				// width:30% !important;
