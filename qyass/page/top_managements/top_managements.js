frappe.pages['top_managements'].on_page_load = function(wrapper) {
    console.log("--- Qyass Top Management: Initializing Full Operations ---");

    // 1. إنشاء الهيكل الأساسي للصفحة
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'الإدارات العليا (Top Management)',
        single_column: true
    });
    ...
    // 3. تشغيل واجهة الإدارات العليا
    new QyassTopManagementDashboard(wrapper, page);
};

class QyassTopManagementDashboard {
    ...
    force_kiosk_mode() {
        const kioskStyle = `
            ...
            .page-container {
				...
				#page-top_managements{padding-top:0px !important;}
...
background:white !important;
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>دليل الإدارات العليا</h1>
                            <p>إدارة واستعراض الإدارات العليا وربطها بالوكالات.</p>
                            <button class="hero-btn" id="btn-new-top-management">
                                <i class="fa fa-plus-circle"></i> إضافة إدارة عليا
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                <div class="stat-icon"><i class="fa fa-building-user"></i></div>
                                <div class="stat-info">
                    ...
                    <div class="list-section">
                        ...
                        <div class="search-box glass-card">
                            ...
                            <input type="text" id="top-management-search" placeholder="بحث عن إدارة أو وكالة...">
                        </div>

                        <div id="top-management-list-wrapper" class="list-grid">
        ...
        // Client-side Search
        layout.find('#top-management-search').on('keyup', (e) => {
        ...
        // Open Create Dialog
        this.body.find('#btn-new-top-management').on('click', () => this.open_create_dialog());
    }
    ...
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            title: 'إضافة إدارة عليا جديدة',
            fields: [
                { label: 'الوكالة', fieldname: 'agency', fieldtype: 'Link', options: 'Agency', reqd: 1 },
                { fieldtype: 'Section Break' },
                { label: 'اسم الإدارة العليا', fieldname: 'top_management', fieldtype: 'Data', reqd: 1, placeholder: 'أدخل اسم الإدارة العليا هنا' }
            ],
            ...
            primary_action: (values) => {
                frappe.call({
                    ...
                    args: { 
                        doc: { 
                            doctype: 'Top Management', 
                            top_management: values.top_management,
                    ...
                    callback: (r) => {
                        if (!r.exc) {
                            d.hide();
                            frappe.show_alert({ message: `تم إضافة الإدارة: ${r.message.top_management}`, indicator: 'green' }, 3);
                            this.fetch_data();
                        }
                    }
                });
    ...
    open_edit_dialog(doc) {
        let d = new frappe.ui.Dialog({
            title: 'تعديل بيانات الإدارة العليا',
            fields: [
                { label: 'الوكالة', fieldname: 'agency', fieldtype: 'Link', options: 'Agency', reqd: 1, default: doc.agency },
                { fieldtype: 'Section Break' },
                { label: 'اسم الإدارة العليا', fieldname: 'top_management', fieldtype: 'Data', reqd: 1, default: doc.name }
            ],
            primary_action_label: 'تحديث البيانات',
            primary_action: (values) => {
                const update_agency = (docname) => {
                    frappe.call({
                        ...
                        freeze: true,
                        freeze_message: 'جاري تحديث الوكالة...',
                        callback: (r) => {
                            if (!r.exc) {
                                d.hide();
                ...
                // إذا تغير اسم الإدارة (والذي يمثل الـ Name في الداتا بيز)، نغير الاسم أولاً ثم نحدث الوكالة
                if(values.top_management !== doc.name) {
                    frappe.call({
                        method: 'frappe.rename_doc',
                        args: { doctype: 'Top Management', old: doc.name, new: values.top_management, merge: 0 },
                        freeze: true,
                        freeze_message: 'جاري تغيير الاسم...',
                        callback: (r) => {
                            if (!r.exc) update_agency(values.top_management);
                        }
                    });
                } else {
                    update_agency(doc.name); // تحديث الوكالة فقط في حال عدم تغيير الاسم
                }
            }
        });
    ...
    open_preview_dialog(doc) {
        let d = new frappe.ui.Dialog({
            title: 'تفاصيل الإدارة العليا',
            fields: [
                { label: 'الإدارة العليا', fieldname: 'top_management', fieldtype: 'Data', read_only: 1, default: doc.name },
                { label: 'الوكالة التابعة لها', fieldname: 'agency', fieldtype: 'Data', read_only: 1, default: doc.agency }
            ]
        });
        
        this.apply_dialog_theme(d, 'fa-eye');
        d.show();
        d.$wrapper.find('.modal-footer').hide(); // إخفاء الفوتر لمنع الحفظ/التعديل
    }

    // ==========================================
    // 4. DELETION
    // ==========================================
    delete_top_management(doc_name) {
        frappe.confirm(`هل أنت متأكد من حذف الإدارة العليا: <b>${doc_name}</b>؟`, () => {
            frappe.call({
                method: 'frappe.client.delete',
                args: { doctype: 'Top Management', name: doc_name },
                freeze: true,
    ...
    // --- Fetch Data ---
    fetch_data() {
        frappe.db.get_list('Top Management', {
            fields: ['name', 'top_management', 'agency', 'modified'],
            limit: 500,
            ...
            console.error(err);
            this.body.find('#top-management-list-wrapper').html(`<div class="alert alert-danger">خطأ: ${err.message}</div>`);
        });
    }

    // --- Render List ---
    render_list(data) {
        const container = this.body.find('#top-management-list-wrapper');
        ...
        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-landmark" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد إدارات عليا</h3>
                    <p>النظام خالي من الإدارات العليا حالياً</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            const title = item.top_management || item.name;
            const agencyName = item.agency || 'غير مرتبط بوكالة';
            ...
            card.find('.btn-delete').on('click', (e) => {
                e.stopPropagation(); 
                this.delete_top_management(item.name);
            });
    ...
    setup_styles() {
        if ($('#qyass-top-management-css').length) return;
        ...
        `;
        $('<style id="qyass-top-management-css">').text(css).appendTo('head');
    }
}