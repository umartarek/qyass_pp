frappe.pages['q&a'].on_page_load = function(wrapper) {
    console.log("--- Q&A Dashboard: Initializing ---");

    // 1. Create the standard page structure
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'الأسئلة الشائعة (Q & A)',
        single_column: true
    });
...
class QADashboard {
    ...
    render_skeleton() {
        const layout = $(`
            <div class="modern-dashboard" dir="rtl">
                ...
                <!-- Navbar -->
                <div class="glass-header" style='background:linear-gradient(135deg, #0369a1 0%, #0ea5e9 100%);'>
                    <div class="brand">
                                            <img src='/files/Screenshot_2026-04-22_at_6.16.56_AM-removebg-preview.png' height='40px' width='50px'/>
                    </div>
                    <div class="user-actions">
                        <button class="action-btn" style='background:white;' onclick="location.href='/app/qyass-dashboard'">
                ...
                <div class="dashboard-container">
                    ...
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>بنك الأسئلة والأجوبة</h1>
                            <p>إدارة قاعدة المعرفة والرد على استفسارات المستخدمين.</p>
                            <!-- Changed to ID for event binding -->
                            <button class="hero-btn" id="btn-new-qa">
                                <i class="fa fa-plus-circle"></i> سؤال جديد
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">إجمالي الأسئلة</span>
                                </div>
                            </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="qa-list-wrapper" class="list-grid">
        ...
        this.body.find('#btn-new-qa').on('click', () => this.open_create_dialog());
    }
    ...
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                },
                {
                    label: 'الإجابة',
                    fieldname: 'answer',
                    fieldtype: 'Text', // Or 'Text Editor' if you want rich text
                    placeholder: 'اكتب الإجابة التفصيلية هنا...'
                }
            ],
            primary_action_label: 'حفظ',
            primary_action: (values) => {
                this.create_qa(values, d);
            }
        });
        ...
        // Add Header Icon
        d.$wrapper.find('.modal-header').prepend('<div class="dialog-icon"><i class="fa fa-question-circle"></i></div>');

        d.show();
    }

    // --- NEW METHOD: Handle API Insert ---
    create_qa(values, dialog) {
        frappe.call({
            ...
            callback: (r) => {
                if (!r.exc) {
                    dialog.hide();
                    frappe.show_alert({
                        message: `<i class="fa fa-check-circle"></i> تم إضافة السؤال بنجاح`,
                        indicator: 'green'
                    }, 5);
    ...
    fetch_data() {
        frappe.db.get_list('Q And A', {
            fields: ['name', 'question', 'answer', 'modified'],
            limit: 200,
            ...
            console.error(err);
            this.body.find('#qa-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#qa-list-wrapper');
        ...
        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-comments" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد أسئلة</h3>
                    <p>أضف السؤال الأول لقاعدة المعرفة</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            let answerPreview = item.answer || 'لا توجد إجابة مسجلة';
            if (answerPreview.length > 80) {
                answerPreview = answerPreview.substring(0, 80) + '...';
            }

            const lastUpdated = frappe.datetime.str_to_user(item.modified);

            // Construct URL
            const targetUrl = `/app/q-and-a/${item.name}`;
                ...
                <div class="modern-list-item glass-card" 
                    ...
                    <div class="item-left">
                        <div class="item-icon-box">
                            <i class="fa fa-question" style="font-size: 1.2rem;"></i>
                        </div>
                        <div class="item-info">
                            <h3 class="item-title">${question}</h3>
                            <div class="item-meta">
                                <span class="meta-text">
                                    <i class="fa fa-align-left"></i> ${answerPreview}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="item-right">
                         <div class="meta-date">
                            <i class="fa fa-clock"></i> ${lastUpdated}
                        </div>
                        <div class="btn-arrow">
    ...
    setup_styles() {
        if ($('#qyass-qa-theme-css').length) return;
        ...
        const css = `
            ...
            .meta-text { color: #64748b; font-size: 0.8rem; display: flex; align-items: center; gap: 4px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; max-width: 400px;}

            .item-right { display: flex; align-items: center; gap: 20px; }
            .meta-date { color: #94a3b8; font-size: 0.8rem; display: flex; align-items: center; gap: 5px; }
            ...
            @media (max-width: 768px) {
                ...
                .meta-text { max-width: 100%; white-space: normal; }
            }
        `;

        $('<style id="qyass-qa-theme-css">').text(css).appendTo('head');
    }
}