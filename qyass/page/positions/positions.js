frappe.pages['positions'].on_page_load = function(wrapper) {
    console.log("--- Positions Directory: Initializing ---");

    // 1. Create the standard page structure
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'المسميات الوظيفية (Positions)',
        single_column: true
    });
    ...
    new PositionDashboard(wrapper, page);
};
...
class PositionDashboard {
    ...
    force_full_screen() {
        const fullScreenStyle = `
            ...
            #page-positions { margin: 0 !important; padding: 0 !important; max-width: 100% !important; width: 100% !important; }
    ...
    render_skeleton() {
        // We attach the open_create_dialog function to the window or handle it via class context
        // For simplicity in HTML string, we'll use a unique ID and attach event listener later
        const layout = $(`
...
background:white !important;
                ...
                <div class="dashboard-container">
                    <div class="hero-section">
                        <div class="hero-text">
                            <h1>دليل المناصب والوظائف</h1>
                            <p>قائمة المسميات الوظيفية المعتمدة في الهيكل التنظيمي.</p>
                            <!-- Changed to ID for event binding -->
                            <button class="hero-btn" id="btn-new-position">
                                <i class="fa fa-plus-circle"></i> منصب جديد
                            </button>
                        </div>
                        ...
                        <div class="hero-stats">
                            <div class="stat-card glass-card">
                                ...
                                <div class="stat-info">
                                    ...
                                    <span class="stat-desc">عدد المسميات</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="list-section">
                        ...
                        <div id="position-list-wrapper" class="list-grid">
        ...
        // Bind the click event to the class method
        this.body.find('#btn-new-position').on('click', () => this.open_create_dialog());
    }
    ...
    open_create_dialog() {
        let d = new frappe.ui.Dialog({
            ...
            primary_action_label: 'حفظ المنصب',
            primary_action: (values) => {
                this.create_position(values, d);
            }
        });
        ...
        // Render custom header icon inside the dialog for visual consistency
        d.$wrapper.find('.modal-header').prepend('<div class="dialog-icon"><i class="fa fa-briefcase"></i></div>');

        d.show();
    }

    // --- NEW METHOD: Handle API Call ---
    create_position(values, dialog) {
        frappe.call({
            method: 'frappe.client.insert',
            args: {
                doc: {
                    doctype: 'Position',
                    position: values.position_name, // Map field to Doctype field
            ...
            callback: (r) => {
                if (!r.exc) {
                    dialog.hide();
                    frappe.show_alert({
                        message: '<i class="fa fa-check-circle"></i> تم إضافة المنصب بنجاح',
                        indicator: 'green'
                    }, 5);
    ...
    fetch_data() {
        frappe.db.get_list('Position', {
            fields: ['name', 'position', 'creation'],
            limit: 500,
            ...
            console.error(err);
            this.body.find('#position-list-wrapper').html(`
    ...
    render_list(data) {
        const container = this.body.find('#position-list-wrapper');
        ...
        if (count === 0) {
            container.html(`
                <div class="empty-state glass-card">
                    <i class="fa fa-id-badge" style="font-size:3rem; opacity:0.3; margin-bottom:15px"></i>
                    <h3>لا توجد مسميات</h3>
                    <p>أضف المسمى الوظيفي الأول</p>
                </div>
            `);
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            const positionName = item.position || item.name;
            const created = frappe.datetime.str_to_user(item.creation).split(' ')[0];
            const targetUrl = `/app/position/${item.name}`;
                ...
                <div class="modern-list-item glass-card" 
                    ...
                    <div class="item-left">
                        <div class="item-icon-box">
                            <i class="fa fa-briefcase" style="font-size: 1.1rem;"></i>
                        </div>
                        <div class="item-info">
                            <h3 class="item-title">${positionName}</h3>
                        </div>
                    </div>
                    <div class="item-right">
                         <span class="meta-text" style="color:#94a3b8; font-size:0.8rem; margin-left: 10px;">
                            ${created}
                        </span>
                        <div class="btn-arrow">
    ...
    setup_styles() {
        if ($('#qyass-position-theme-css').length) return;
        ...
        const css = `
            ...
            .item-icon-box {
                width: 40px; height: 40px; background: linear-gradient(135deg, #0369a1 0%, #0ea5e9 100%);
                color: white; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: 700;
            }
            ...
            /* Remove Frappe default gray bg from footer if present */
            .qyass-theme-dialog .standard-actions { background: transparent; }

            @media (max-width: 768px) {
                .modern-list-item { flex-direction: row; align-items: center; }
        ...
        $('<style id="qyass-position-theme-css">').text(css).appendTo('head');
    }
}